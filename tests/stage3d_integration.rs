...
fn test_search_partial_name() {
    ...
    assert!(
        !results.is_empty(),
        "search for 'handle' should find handle_request"
    );
...
fn test_search_results_have_context() {
    ...
    assert!(
        main_result.community_id.is_some(),
        "main should have a community"
    );
    let _ = fs::remove_dir_all(&tmp_root);
}
...
fn test_get_context() {
    ...
    let qr = store
        .execute_query("MATCH (f:Function) WHERE f.name = 'main' RETURN f.qualified_name")
        .unwrap();
    ...
    assert!(
        ctx.community.is_some(),
        "main should have community context"
    );
    assert!(
        !ctx.processes.is_empty(),
        "main should participate in processes"
    );
