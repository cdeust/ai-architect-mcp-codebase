...
| `Calls` | Function, Method | Function, Method | Parse function bodies for `call_expression` nodes (tree-sitter). Extract callee path. If fully qualified (`foo::bar::baz()`), resolve directly against `qualified_name`. If unqualified (`baz()`), resolve via the file's import graph (follow `Imports` edges to find `baz` in scope). A same-class receiver on a `Method` caller — Rust `self.<m>` / `Self::<m>` (issue #283 lot 4), Python `self.<m>` and TypeScript `this.<m>` (issue #290) — resolves against the caller's enclosing type (`resolver::receiver`, `Evidence::ReceiverBound`) before either of those paths. | Partially — fully-qualified and import-resolved calls: yes. Same-class receiver calls (Rust `self`/`Self`, Python `self`, TypeScript `this`): yes (receiver-type evidence). Method calls on locals with a single explicit binding: yes (issue #283 lot 6, deferred). Method calls on inferred types: no. |
...
- **Method calls on inferred types:** `let x = get_thing(); x.do_it();` — requires type inference. (Same-class receivers — Rust `self.<m>` / `Self::<m>`, Python `self.<m>` and TypeScript `this.<m>` since issue #290 — are no longer in this bucket: they resolve statically via `resolver::receiver`, `Evidence::ReceiverBound`. Neither are Rust locals bound exactly once to a named type (issue #283 lot 6, `Evidence::ReceiverLocalBinding`). A local whose binding is itself type-inferred, e.g. `let x = get_thing();` above, stays deferred to LSP.)
