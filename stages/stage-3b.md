...
| `Calls` | Function, Method | Function, Method | Parse function bodies for `call_expression` nodes (tree-sitter). Extract callee path. If fully qualified (`foo::bar::baz()`), resolve directly against `qualified_name`. If unqualified (`baz()`), resolve via the file's import graph (follow `Imports` edges to find `baz` in scope). For Rust, a `self.<m>` / `Self::<m>` receiver on a `Method` caller resolves against the enclosing `impl` type (`resolver::receiver`, `Evidence::ReceiverBound`, issue #283 lot 4) before either of those paths. | Partially — fully-qualified and import-resolved calls: yes. Rust `self`/`Self` receiver calls: yes (receiver-type evidence). Method calls on locals with a single explicit binding: yes (issue #283 lot 6, deferred). Method calls on inferred types: no. |
...
- **Method calls on inferred types:** `let x = get_thing(); x.do_it();` — requires type inference. (Rust `self.<m>` / `Self::<m>` receivers, and — issue #283 lot 6 — locals bound exactly once to a named type, are no longer in this bucket: they resolve statically via `resolver::receiver`, `Evidence::ReceiverBound`. A local whose binding is itself type-inferred, e.g. `let x = get_thing();` above, stays deferred to LSP.)
