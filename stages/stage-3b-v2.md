...
- **Layer 4 has two kinds of entry (issue #339).** A fixed expansion (`println!` always calls `_print`) lists targets that are all called, and each is an edge. A decided expansion is a macro whose target depends on something at the site: `write!` and `writeln!` expand to `dst.write_fmt(..)`, chosen by the declared type of `dst`, resolved in the scope of the file that holds the site (`fmt::Formatter` has an inherent method, `String` uses `fmt::Write`, the io types use `io::Write`; a type defined in the file or imported from a non-std path is never the std one, an alias resolves through its original path, and a named type that nothing in the file places, or that two `use` items bind differently, stays undetermined), and `vec!` by its argument shape (`vec![]` is `Vec::new`, `vec![x; n]` is `vec::from_elem`, a list expands to compiler internals and has no target). The parser records `receiver_hint` (the destination's type as written) and `macro_arg_shape` on the site; the resolver places the type in the scope of the file (confidence 0.8, method `macro-expansion-receiver-type`) and otherwise gives up: the write traits a file imports decide nothing, because what is in scope says what the file may call, not what the destination is. A site with no determined target gets no edge and no per-site row, stays `is_resolved = false`, and is reported unresolved with `destination type not determined`, `callee depends on the arguments of the macro` or `expansion calls compiler internals whose paths change between versions`. A decided expansion counts as one reference in `total_refs`. Each resolve first deletes the macro-expansion rows of earlier runs, so the graph follows the current rules. Macro sites are not sent to the language server; `lsp_resolve` counts them apart as `macro_sites_count`.
- **Layer 4 has two more classes (issues #344, #345).** The rule for a listed target: an entry names the one callee that every form of the macro reaches, checked in the std sources of the pinned toolchain and in the expansion of the supported compilers (1.93 to 1.98). `println!` and `print!` call `_print`, `eprintln!`, `eprint!` and `dbg!` call `_eprint`, `format!` calls `fmt::format`, and the four comparison asserts (`assert_eq!`, `assert_ne!`, `debug_assert_eq!`, `debug_assert_ne!`) call `assert_failed` in every arm. `Arguments::new_v1` is not listed: the compiler no longer emits it. `panic!`, `assert!`, `debug_assert!`, `todo!`, `unimplemented!` and `unreachable!` fail the rule: `panic` or `panic_fmt` is called depending on the arguments and the edition, so their site is an unresolved reference with the reason "callee depends on the arguments of the macro". The list form of `vec!` expands to compiler internals whose paths change between versions and has the reason "expansion calls compiler internals whose paths change between versions". A macro that calls nothing (`matches!`, `include_str!`, `include_bytes!`, `concat!`, `stringify!`, `env!`, `option_env!`, `cfg!`, `line!`, `file!`, `column!`, `module_path!`) is not a reference: its site gets no edge and no per-site row, stays `is_resolved = false` (true would claim a target the graph does not have, the defect #335 fixed), is in neither `total_refs` nor `unresolved`, and is counted as `no_call_macro_sites` in the `resolve_graph` result and the `analyze_codebase` `resolve` block. It is also left out of `lsp_status.macro_sites_count`. `include!` is not in the class: it splices code that can call. Calls written inside the arguments or the guard of one (`matches!(f(x), ..)`, `matches!(x, Some(v) if pred(v))`) are still extracted as call sites of the enclosing function. Every resolve deletes the `StdlibSymbol` nodes left with no relationship after the purge of old macro rows.
- **Layer 3 has a receiver tier read off a return type (issues #348, #349).** A Rust local with no written type, initialised by a call of a free function (`let s = make();`), takes the type from that function's declared return type, at confidence 0.85 and with `resolution_method: "receiver-return-type"`, one step weaker than `"receiver-local-binding"` (0.87), which reads a type written at the binding. The value is measured: on dy-wcet v4.1.6 (static run of this build) the tier added 43 per-site rows, none of the existing rows changed, and each of the 43 names the target that the language-server run of the official 0.13.0 binary names for the same site (that run is the earlier measurement, `tasks/axon-v4.1.6-20260924/raw/lsp-standalone-sites`); the 14 `response_of` sites among them are also right against the hand-built oracle. It ties `SameFileUnique` (0.85) without effect, since this tier applies only to a receiver call (`x.m()`) and that one to a bare-name lookup. An `Option` or `Result` return is unwrapped only through a form the source spells out: `let Some(s) = .. else`, `let Ok(s) = .. else`, `.expect(..)`, `.unwrap()` or `?`. The parser reads the callee from its own file only, and declines on doubt: the name must denote exactly one free function of the file, declared in a scope that encloses the call, with no `use`, tuple struct, `const` or `static` of that name in the file and no local of that name; the return type must be a plain named type that is not one of the function's own generic parameters, not `impl Trait`, `dyn Trait` or `Self`; and the receiver must be bound exactly once in the function, counting every binding form: `let`, parameters (nested functions included), closure parameters, `if let`, `while let`, `match` arms and `for` patterns, and any name a macro may bind (an unknown macro handed the name, or a std macro whose token tree holds a binding token). A `let` binds only where its enclosing block contains the call and the `let` ends before it; a `const`, `static`, const generic or `use` of the name inside the function declines it. A return type with a path (`other::Set`), that is a `type` alias in the file or anywhere in the graph, or that the file renames with `use x::Real as Local`, is declined. The name of the return type must be shown by the file, in the module of the function (Rust resolves a bare name there: the module's own definitions and imports, its globs, then the prelude, and never those of another module, the root included): a struct, enum or union of that name defined in that module (it shadows every glob), an explicit `use` that binds that name in that module (it wins over a glob; only the last segment of a path or the leaf of a list binds it, so `use Set::{A, B};` and `use ext::Set::Variant;` do not, and `use ext::Set as Other;` binds `Other`), or only `use super::*;` globs, in which case the same test is applied to the parent module. An explicit `use` is trusted only with evidence from the repository, never from the name: a path that starts with `crate`, `self` or `super` is accepted, and a path that starts with another name is accepted only when that name is a library crate of the analysed workspace. The parser marks such a hint `CallSite.receiver_hint_via = "return-type-import:<first segment>"`; the indexer, which already runs `cargo metadata --no-deps` for the coverage report, reads the library targets (`packages[].targets[]` of kind `lib`, `rlib`, `dylib`, `cdylib`, `staticlib` or `proc-macro`; the target `name`, so a `[lib] name` override is honoured, with hyphens as underscores, every member of a workspace) and rewrites the hints whose first segment is one of them to `"return-type"`, in the full, incremental and bootstrap-fill paths. The resolver declines every hint left with the `return-type-import:` prefix, so a repository without a readable `Cargo.toml`, a failed `cargo metadata`, an old graph or a foreign crate all give no edge. Limits: a path that starts with a crate of the repository is accepted even when the type is re-exported from a foreign crate; a name that reaches the module through a glob import is not seen; `extern crate x as y;` is not seen; a repository with a nested or absent root `Cargo.toml` knows no crate name. Anything else is declined: a glob of another path (`use a::*;`, `use a::{b::*, c};`, `use crate::x::*;`), no import at all, a homonym defined only in a nested module or at the root of a function in `mod m`, and `super::*` at the file root. The cost is a lost edge for a file that globs an in-repo module holding exactly one such type; on dy-wcet v4.1.6 it is nil. Two limits of the alias rule: `extern crate x as Set;` is not seen, and an alias declared in another module of the file declines a same-named return type elsewhere in it. The same count now backs the written-type tiers, which a pattern rebinding the name used to leave typed. A callee in another file, and a receiver rebound by a later `let` of the same name, stay unresolved for the language server. The parser marks such a hint with `CallSite.receiver_hint_via = "return-type"`; the resolver relabels the palier-3 result and never adds an edge the palier-3 lookup did not find.
...
### 5.1 Rust items under mutually exclusive `#[cfg]` (issue #353)

Two items of one label and qualified name in one file under different `#[cfg]` gates (`#[cfg(feature = "fast")] fn pick` and `#[cfg(not(feature = "fast"))] fn pick`) are two nodes. Only an item that collides is renamed: its id and qualified name gain `#cfg(<compact gate>)` (`src/lib.rs::pick#cfg(not(feature=fast))`), and the node carries the gate in `cfg_gate`. Every other id is unchanged. The gate is the `all` of the item's own `#[cfg]`, those of its enclosing `mod`, `impl`, `trait` and `fn`, and the inner `#![cfg]` of the file or of an enclosing inline module, in a canonical order (nested `all`/`any` flattened, duplicates removed, one-term lists unwrapped, `not(not(x))` folded), with comments removed and spaces and quotes dropped. `cfg_attr` is not expanded: a `cfg` added through `cfg_attr` gates nothing here. Because the gate includes the enclosing gates, moving a `#[cfg]` from a `mod` to its items renames the ids of the items that are twins, and an item that becomes a twin changes from its plain id to the suffixed one; the incremental refresh keeps an incoming cross-file edge only when its target id still exists after the reparse (an edge to an id that vanished is skipped), and the next resolve derives the edges again; for twins it derives none. The flag `is_resolved` of a call site in an unchanged file is not touched by the refresh (it is the index stage), and a resolve that finds only twins sets it to false.

A call whose candidates are all twins of one item is resolved only when the build decides which twin it compiles, and otherwise gets no edge and the reason `cfg_twins` on its call site. The same holds for every edge whose target is looked up by name: `Implements` (impl blocks and derives), `Uses`, which never choose a twin. An `impl` block for a type that has twins belongs to a twin only when the block's own gate is exactly that twin's gate; otherwise it has no `HasMethod` edge.

The build decides in two ways, both syntactic, and a twin is chosen only when exactly one twin is not ruled out and that twin is shown compiled (a lone twin that is merely undecided is not chosen, because two twins are not guaranteed to be complementary). (1) The caller's own gate: a caller whose id carries `#cfg(..)` (it is a twin, or sits in a twin `mod`, `impl` or `trait`) exists only when that gate holds, so among the twins it reaches the one whose gate every conjunct of the caller's gate contains, and never one whose gate the caller contradicts by holding the negation of a conjunct. A caller under `#[cfg(kani)]` that has no twin carries no gate in its id, so it decides nothing. (2) The default profile: after each index pass (full, incremental, bootstrap fill) every twin gets `cfg_active` = `active`, `inactive` or `unknown`, from the features `cargo metadata` reports as enabled by default for the package that compiles its file, with the three-valued evaluation of `cfg_expr`: only `feature = ".."` leaves are decided, so `kani`, `test`, `unix` and `target_*` stay `unknown` and a twin behind them is left open; a file reached only through `mod` declarations the default features compile out holds only `inactive` twins; a file two packages compile with different features, a file no crate root reaches, an unreadable gate and a repository without a readable `Cargo.toml` give `unknown`. The pass rewrites every twin every time, because a `Cargo.toml` edit changes the value without changing a file. The chosen edge has `resolution_method: "cfg-selected"` at confidence 0.85 (`Evidence::CfgSelected`; a policy value, below `import-scope-lookup`, level with `same-file-unique`, and never above 0.9 because it also assumes that the build under analysis is the default one), and every resolve first deletes the `cfg-selected` rows of the earlier run and reopens their sites, so an edit of the features moves the edge. `cfg_active` is a property of the default profile only. `BuildProfile` (`parser/cfg_expr.rs`) is where a second profile would plug in: a Kani build is a profile that decides the option `kani`, and would need its own labelled edges; none is written now. `get_impact` and `get_symbol` accept the bare name of a twinned item and resolve it to the one twin that is `active` (otherwise they list the twins), show `cfg_gate`, `cfg_active` and `cfg_twins`, and `get_impact` is never exact for a twin; `index_status` reports `cfg_twins` (`sets`, `members`, per-state counts, `unresolved_sites`) and lists the twins the default build compiles out under `coverage.feature_gated.items`. Not covered: twin files selected by `#[cfg_attr(.., path = "..")]`, twins in different files (an ordinary ambiguity), and the language-server tier, which may still point at a twin the default build does not compile. A graph indexed before this change has already lost its twins, and a graph whose `GraphMarker` row of the canonical form is missing or older names them by an older form (`CANONICAL_FORM_VERSION`, bumped whenever the canonical form of a gate changes), so an incremental refresh of it asks for a full reindex. A call site of another file whose per-site resolution edge (`Calls_CallSite_*`) points into a file that an incremental refresh deletes, renames or rewrites is reopened by that refresh (`indexer/incremental/stale_sites.rs`: flagged `is_resolved = false` before the purge, set back to true after the relink if the edge is restored); a site with no per-site edge (a tuple-struct constructor call, issue #356) or resolved by the macro or language-server tier keeps its flag.
