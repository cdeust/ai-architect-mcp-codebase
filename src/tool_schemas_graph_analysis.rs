...
pub(super) fn lsp_resolve_schema() -> Value {
    json!({
        "name": "lsp_resolve",
        "description": "Stage 3b-v2 — LSP-enhanced resolution. Queries a Language Server Protocol server (rust-analyzer, pyright, typescript-language-server) to resolve method calls on inferred types that the static resolver cannot handle. Runs AFTER resolve_graph. Requires the LSP server to be installed; gracefully fails if not found. Issue #282: if the target sits under a parent workspace that does not list it as a member (or another condition the server reports as `health: \"error\"`), the phase fails loudly as an `lsp_workspace_load_failed` error instead of silently answering every request `[]`. On success, the response's `state` distinguishes `completed` (at least one site resolved, or nothing needed resolving) from `completed_unresolved` (the pass ran but resolved nothing); `server_health` carries the language server's own last-reported health. `outside_targets_count` (issue #284) is sites the pass proved sit outside every compiled Cargo target (see `query_graph(graph=\"missed\").coverage.outside_build_targets`) and therefore never got a `textDocument/definition` request — counted separately from `failed_count` (a real negative answer) and `skipped_count` (budget ran out); beside at least one resolved site it does not flip `state`, but when no site resolved and every one is outside targets `state` is `completed_unresolved` (issue #315). `macro_sites_count` (issue #339) is the macro-invocation sites (`write!`, `vec!`, `panic!`, ...) still unresolved after the static phase: they are never sent to the server, whose answer is the macro's own definition, and they sit outside the partition above. A macro that calls nothing (`matches!`, `include_str!`, issue #345) is not in it: it is not a call, and `resolve_graph` reports those sites as `no_call_macro_sites`. `unlinked_file_check` (issue #292) reports rust-analyzer's own `unlinked-file` verdict for each file the pass opened, beside the cargo attribution (`cargo_attribution`, and `linked_despite_outside_build_targets` for the opposite disagreement); `pull_supported: false` means the server offered no pull diagnostics and nothing was checked. Unlinked files are merged into the graph's coverage sidecar as `unlinked_file` (`coverage_merge`: merged | nothing_to_merge | unavailable when no sidecar exists). DURABILITY (issue #352): before answering, the call closes its own graph handle, reopens the graph and counts the lsp-definition rows again; persisted.lsp_rows is that durable count, and the call fails with lsp_rows_not_durable when it differs from what the pass wrote, so a completed answer always means the rows are there for the next reader.",
