// parser::spec::guard_refs — WHICH strings each spec row references (ADR-0055
// Consequences).
//
// One function per row shape, each returning `(field_name, string)` pairs so the
// guard's failure message can name the exact spec field that went stale. This is
// the enumeration half of the spec-validation guard; the sibling `guard` module
// holds the grammar-JSON side (which `node-types.json` belongs to which
// language, including a dialect's second grammar) and the tests that compare the
// two.
//
// Split out of `guard.rs` along that boundary (§4.1): every migrated language
// adds a sub-table block here, and with the TypeScript family the single file
// reached 532 lines. Adding a language now grows only this file.
