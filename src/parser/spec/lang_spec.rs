...
use super::families::{CFamilySpec, CppFamilySpec, ObjcFamilySpec, TsFamilySpec};
...
/// An alternate grammar dialect selected by file extension.
///
/// One `Language` can be served by two grammars that differ in accepted syntax.
/// TypeScript is the case: tree-sitter-typescript ships a `typescript` grammar
/// and a `tsx` grammar, and JSX (`<Component/>`) parses ONLY under `tsx` — under
/// `typescript` every JSX element becomes an ERROR node and the symbols inside
/// it are dropped. The repo also routes the JS family (`.js`/`.jsx`/`.mjs`/
/// `.cjs`) to `Language::TypeScript`; JS carries no type syntax, so `tsx` is
/// safe (and necessary, for `.jsx`) for those too.
///
/// `parse_with_spec` picks this dialect when the file path's extension is in
/// `extensions`, else the row's default `ts_language`. The spec guard validates
/// every node kind against BOTH grammars' `node-types.json`, so a row can never
/// name a kind that only one dialect knows.
/// source: tree-sitter-typescript 0.23.2 (`typescript/src/node-types.json` vs
/// `tsx/src/node-types.json`; `LANGUAGE_TYPESCRIPT` vs `LANGUAGE_TSX`).
pub(crate) struct DialectSpec {
    /// Extensions (the text after the last `.`, case-sensitive) selecting this
    /// dialect. Matched against the parsed file's path.
    pub extensions: &'static [&'static str],
    /// The alternate grammar factory.
...
pub(crate) struct LangSpec {
    ...
    /// When `Some`, this language is an ECMAScript-family grammar (TypeScript
    /// now): `walk_defs` delegates the whole file to the `ts` walker (consuming
    /// this sub-table) instead of the C-family walkers or the class-model arms.
    /// `None` for every other language. At most one of `c_family` /
    /// `cpp_family` / `ts_family` is `Some` on a given row.
    pub ts_family: Option<&'static TsFamilySpec>,
    /// An alternate grammar selected by the parsed file's extension, or `None`
    /// when the language has exactly one grammar. Read by `parse_with_spec`
    /// (grammar selection) and by the spec guard (which validates every node
    /// kind against this dialect too).
    pub dialect: Option<&'static DialectSpec>,
}
