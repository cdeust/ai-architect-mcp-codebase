// parser::spec::guard::collect — the string-COLLECTION half of the spec guard:
// given a spec row, enumerate every node kind and field name it references, each
// tagged with the field it came from so a failure names the culprit.
//
// Split from `guard/mod.rs` (which holds the assertions) along that concern
// boundary, and to keep both files under the 500-line cap (§4.1). Adding a
// language, a sub-table, or a field means adding a row HERE and nothing in
// `mod.rs` — the OCP shape the guard already had, now made structural.
