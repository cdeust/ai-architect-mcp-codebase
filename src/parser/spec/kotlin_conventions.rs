// parser::spec::kotlin_conventions — Kotlin's behavioral escape hatch
// (ADR-0055 §4): the predicates and shaping that will not reduce to node-kind
// slices — modifier-keyword visibility, one-kind-many-concepts class
// classification, `delegation_specifiers` inheritance, property/enum-entry
// member constants, and the navigation-expression call callee.
//
// Split out of `kotlin.rs` (which keeps the module doc and the KOTLIN_SPEC data
// row) along the data/behavior boundary the ADR draws, to hold both files under
// the §4.1 500-line cap. Pure move: the Kotlin parity corpus is unchanged.
...
use crate::parser::{node_text, qual, LABEL_ENUM, LABEL_STRUCT, LABEL_TRAIT};
