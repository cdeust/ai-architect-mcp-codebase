// parser::spec::kotlin_conventions — the KotlinConventions behavioral override
// (ADR-0055 phase 4, the ADR Risk-1 watch surface). Split out of `kotlin.rs`
// so the KOTLIN_SPEC data row and this behavior each stay under the §4.1
// 500-line cap; the design rationale for every override lives in `kotlin.rs`'s
// module doc. Pure move — no behavior changed.
...
impl LanguageConventions for KotlinConventions {
    ...
    fn call_entry(
        ...
        CallEntry {
            ...
            properties: vec![
                ("callee_name".to_string(), repr.clone()),
...
pub(super) static KOTLIN_CONVENTIONS: KotlinConventions = KotlinConventions;
