...
// The behavioral half of this row (`KotlinConventions` and the Kotlin-grammar
// node kinds only IT reads) lives in the sibling `kotlin_conventions` module,
// along the data/behavior boundary ADR-0055 §4 draws.
use super::kotlin_conventions::KotlinConventions;
...
pub(crate) static KOTLIN_SPEC: LangSpec = LangSpec {
    ...
    objc_family: None,
    ts_family: None,
    // One grammar per Kotlin row: no extension-selected dialect.
    dialect: None,
};

// The `qualifier_or_tail` unit test moved with the function itself, into
// `kotlin_conventions`.
