...
pub(crate) fn do_get_impact(arguments: &Value) -> Result<Value, String> {
    ...
    // fleet-watch#19: resolve the caller's qualified_name to the stored form
    // before the impact query, exactly as get_symbol/get_context/check_gates
    // do. clustering::get_impact does only strict `n.id`/`n.qualified_name`
    ...
    // empty result instead of an error. Resolve first; on a genuine miss return
    // `symbol_not_found` with suggestions, like the peer tools, never a vacuous
    // "exact" blast radius.
    ...
    let mut impact = clustering::get_impact(&store, &resolved_qn)?;
    sort_impact_lists(&mut impact);

    let sections = paged_impact_sections(&impact, offset);
    let mut out = impact_envelope(&sections, &impact, qn, args);
    attach_cochange_section(&mut out, &store, qn, args);

    // Suggested follow-up traversals from this blast-radius result.
    out["next_steps"] = impact_next_steps(&impact, qn);

    attach_foreign_callers(&mut out, arguments, graph_path, qn);
    Ok(out)
}
...
/// same detail/format surface. The symbol's file is the qn up to '::'.
/// Partners have their own shape ({file, cochange_count, coupling, jaccard}),
/// so they render with their own columns/id — NOT the impact columns.
