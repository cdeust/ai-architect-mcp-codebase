...
pub(crate) fn do_get_symbol(arguments: &Value) -> Result<Value, String> {
    ...
    let resolved_qn = match search::resolve_qualified_name(&store, qn) {
        Ok(q) => q,
        Err(nf) => return Ok(unresolved_symbol_response(arguments, graph_path, qn, nf)),
    };
    ...
    Ok(json!({
        ...
        "status": "ok",
        "tool": "get_symbol",
        "node": find_symbol_node(&store, &escaped)?,
        "edges_out": find_symbol_edges_out(&store, &escaped)?,
        "edges_in": find_symbol_edges_in(&store, &escaped)?,
        "next_steps": [
...
///
/// Cross-repo bridge: a symbol absent locally may be DEFINED in a sibling repo
/// (the dangling reference the resolver marks external). Consult siblings
/// before declaring it not-found. Absent the arg this is a no-op and the
...
pub(crate) fn do_resolve_graph(arguments: &Value) -> Result<Value, String> {
    ...
    attach_cross_repo_resolvable(&mut out, arguments, graph_path, &result);
    Ok(out)
}
...
fn attach_cross_repo_resolvable(
    out: &mut Value,
    arguments: &Value,
    graph_path: &Path,
...
/// A relationship table as (table name, from label, to label).
pub(crate) type RelTriple = (&'static str, &'static str, &'static str);
...
///
/// A `const` table rather than a function body returning a literal: this is
/// data, and stating it as data is both the honest construct and what keeps
/// the accessor below inside the §4.2 function-length cap.
const REL_TABLE_TRIPLES: &[RelTriple] = &[
    // 3a tables
...
/// The relationship-table triples, as the edge lookups consume them.
pub(crate) fn rel_table_triples() -> &'static [RelTriple] {
    REL_TABLE_TRIPLES
}
