        ...
        "inputSchema": {
            ...
            "properties": {
                ...
                "graph": {
                    ...
                    "enum": ["default", "missed"],
                    "default": "default",
                    "description": "Which view to query. 'default': run the Cypher 'query' against the code graph. 'missed' (issue #57): ignore 'query' and return the coverage report — the files the index could NOT fully cover (parse_incomplete with flagged line ranges, skipped, quarantined, outside_build_targets — issue #284, Rust files outside every compiled Cargo target, feature_gated — issue #291, Rust modules compiled out by a default-disabled feature), so an agent can pivot to grep. Absence from the missed list is NOT a completeness guarantee."
                },
                "query": {
                    "type": "string",
