...
pub(super) fn index_status_schema() -> Value {
    json!({
        "name": "index_status",
        "description": "Stage 3a — Report an indexed graph's status and its indexing-COVERAGE (issue #57): node_count, edge_count (relationship rows that state a fact of their own; the per-site call rows are not in it), call_site_target_count (the per-site rows, one per resolved call site and target; issue #338), files indexed, and which files the indexer could NOT fully cover — 'parse_incomplete' (WERE indexed, but tree-sitter left ERROR/MISSING line ranges, so constructs there MAY be missing from the graph — grep those ranges), 'skipped' (not indexed at all: oversized/read-failure/parse-timeout), 'quarantined' (the parser panicked; isolated so it could not kill the index), 'outside_build_targets' (issue #284, Rust only: WAS indexed — declarations are in the graph — but the file sits outside every compiled Cargo target per `cargo metadata --no-deps`, e.g. a Kani proof harness, so calls out of it cannot be resolved by the language server), 'unlinked_file' (issue #292, Rust only, recorded by an LSP pass: rust-analyzer itself reported the file is in no crate of its crate graph), and 'feature_gated' (issue #291, Rust only: WAS indexed, inside a compiled target, but every `mod` declaration reaching it is behind a `#[cfg]` false under the package's default features, so the default build and the language server compile it out; the reason names the gate). 'cargo_attribution' (issue #316) says whether those two Cargo-derived buckets carry information: 'known', 'unknown' (a Cargo.toml exists but `cargo metadata` failed or cargo is not on PATH — empty buckets then mean NOT DETERMINED, and 'detail' names the cause; get_impact never reports exact under it), 'not_applicable' (no root Cargo.toml or no .rs file), or 'not_recorded' (a record older than the field). Counts are exact; example lists are capped (full lists in the index_coverage.json sidecar). IMPORTANT: absence of a flag is NOT a completeness guarantee — the signal only marks what the indexer can detect. Use before trusting graph completeness on a file; for a structural view of the misses use query_graph(graph=\"missed\"). TWINS (issue #353): 'cfg_twins' counts the Rust items that exist twice under mutually exclusive #[cfg] gates (each twin is a node named `<qualified_name>#cfg(<gate>)`): 'sets', 'members', how many the DEFAULT build profile (default features from cargo metadata) compiles ('active'), compiles out ('inactive') or does not decide ('unknown': kani, test, unix and target options are never decided), and 'unresolved_sites', the call sites left open because no twin is shown compiled. 'coverage.feature_gated.items' lists the twins the default build compiles out (count exact, examples capped), next to 'files' for gated modules.",
...
pub(super) fn query_graph_schema() -> Value {
    json!({
        "name": "query_graph",
        "description": "Stage 3a — Run a read-only Cypher query against the code graph: the ESCAPE HATCH for structural questions the typed tools do not cover (arbitrary MATCH patterns, aggregations, multi-hop traversals). USE THIS INSTEAD OF scripting grep across files for structural facts like 'which functions call X and live in module Y'. Read-only: mutation keywords are rejected. Response: 'columns' + 'rows' (rows as compact arrays), a human-readable 'result' table, 'total_count', 'truncated', 'order_stable', and 'next_offset' — page a large result by re-calling with offset=next_offset, but paging is cursor-safe ONLY when your query declares ORDER BY (order_stable reports whether it does). ROW BOUND (issue #334): a query with no LIMIT of its own is bounded to 'row_limit' rows per page (500; 'limit_injected': true). 'truncated' is true whenever more rows exist after this page, whether that row bound or the response byte budget stopped it; follow next_offset to read them. When the row bound cut the result, 'total_count' counts only the rows up to next_offset, so it is a lower bound, not the result size. A LIMIT you declare is your own bound: rows past it are never reported as truncated. TOKEN SURFACE (issue #56): format='tabular' drops the redundant human 'result' string (rows+columns already carry the data); detail='ids' collapses to the first column's values for a cheap sweep. COVERAGE HONESTY (issue #57): pass graph=\"missed\" to enumerate what the index does NOT cover (parse-incomplete + skipped + quarantined + outside_build_targets files, issue #284 — Rust files outside every compiled Cargo target — + unlinked_file, issue #292 — files rust-analyzer reports as in no crate — + feature_gated files, issue #291 — Rust modules compiled out by a default-disabled feature; cargo_attribution, issue #316, says whether the two Cargo-derived buckets were determined at all) so you know where to prefer grep. IMPORTANT: absence from graph results — or from the 'missed' list — is NOT a completeness guarantee; the signal only marks what the indexer can detect. COLUMNS OF TWINS (issue #353): Function, Method, Struct, Enum, Variant, Trait, Field, Constant, TypeAlias and Module nodes carry 'cfg_gate' (the compact #[cfg] gate of a twin item, '' for every other item) and 'cfg_active' ('active', 'inactive' or 'unknown' under the default build profile, '' when not decided or on a graph written before the column); a call resolved among twins has resolution_method 'cfg-selected' and confidence 0.85, and an open call site carries unresolved_reason 'cfg_twins'.",
        "annotations": { "readOnlyHint": true },
        "inputSchema": {
            "type": "object",
            "required": ["graph_path"],
            ...
            "properties": {
                ...
                "graph": {
                    ...
                    "enum": ["default", "missed"],
                    "default": "default",
                    "description": "Which view to query. 'default': run the Cypher 'query' against the code graph. 'missed' (issue #57): ignore 'query' and return the coverage report — the files the index could NOT fully cover (parse_incomplete with flagged line ranges, skipped, quarantined, outside_build_targets — issue #284, Rust files outside every compiled Cargo target, unlinked_file — issue #292, files rust-analyzer reports as in no crate, feature_gated — issue #291, Rust modules compiled out by a default-disabled feature; cargo_attribution — issue #316, unknown when cargo metadata failed so those two buckets were never determined), so an agent can pivot to grep. Absence from the missed list is NOT a completeness guarantee."
                },
                "query": {
                    "type": "string",
