...
| §4.1 File size (500) | The generic walker must stay < 500 LOC. The spec registry is *data* (analogous to CBM's 170 KB table) — one row-module per language keeps any single file small; a data table that exceeds 500 lines is marked `// data table` per the §4.1 auto-generated exception spirit. | **Pass** — the size-watch note fired and was resolved: the walker reached 880 lines across the Go/Python/Java/Kotlin/Swift migrations and was split along concern boundaries into `walkers/{mod,defs,calls,imports,embedded,types,constants}.rs` (issue #101, largest file 265 lines). |
